...
GAME.Tower = {
  ...
  //  ── 내 소환수가 층을 따라가는 배수 (2026-09-04) ────────────────────────────
  //  태현님: "주술사 유닛이 너무 약해 나오자마자 1방에 죽어버려."
  //  적 유닛은 `unitModsFor` 로 층·성장 배수를 받는데 **내 소환수는 UNITS 원본에
  //  묶여 있었다.** 실측(고성장 90층): 적 한 대 195 vs 소환 전사 체력 192 — 한 대다.
  //  적 체력이 13.3배로 자라는 동안 소환수는 1.0배였다.
  //
  //  ⚠ **두 축을 엇갈려 곱한다.** 소환수 체력은 `적 공격 배수`를, 소환수 공격은
  //    `적 체력 배수`를 따라간다. 그래야 "몇 대 버티는가 / 몇 대에 죽이는가"가
  //    층·성장과 무관하게 보존된다 — 위 hpMul/dmgMul 이 적을 나에게 맞추는 것의 거울이다.
  //    같은 축끼리 곱하면(체력↔체력) 한쪽으로 몰아준 빌드에서 소환수가 같이 기울어
  //    결국 다시 무의미해진다.
  //  ⚠ 이 값은 **소환수의 절대 강함이 아니라 상대적 유용성**을 고정한다. 그래서 이
  //    수정은 난이도를 올리지도 내리지도 않는다 — 1층에서의 쓸모가 그대로 유지될 뿐이다.
  ...
  summonModsFor: function (floor, qualityMul) {
    var m = this.modsFor(floor);
    var q = qualityMul > 1 ? qualityMul : 1;
    return {
      hp: (m.damage || 1) * Math.pow(q, 0.75),      // 적 공격 배수 ← 버티는 대수 보존
      damage: (m.hp || 1) * Math.pow(q, 1.25)        // 적 체력 배수 ← 죽이는 대수 보존
    };
  },
