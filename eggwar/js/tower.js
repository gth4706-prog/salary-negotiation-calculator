...
GAME.Tower = {
  ...
  //  내 소환수에 걸리는 지수 — **1.0(정비례)**. 적 유닛(0.75)보다 높게 잡은 이유는
  //  태현님 지시("소환수는 층을 따라가기보단 능력치를 더 따라가게")를 그대로 옮긴 것이다.
  //  1.0 이면 소환수가 **내 능력치의 일정한 몫**으로 고정된다 — 내가 두 배 세지면
  //  소환수도 정확히 두 배. 1 을 넘기면 소환수가 나보다 빨리 자라 결국 내가 아니라
  ...
  //  ⚠⚠ **2026-09-07 「⏫ 도약」이 들어오면서 0.75 → 0.38 로 내렸다(위아래 대칭).**
  //    0.75 는 아래 지수(POWER_POW_UNIT)와 같아서 곱을 풀면
  //        기대^0.75 × (내것/기대)^0.75 = 내것^0.75
  //    이 되어 **과성장한 사람에게는 층이 사라진다.** 실측: 공격지수 459 인 사람의
  //    적 체력 배수가 42층도 200층도 **99.2 로 같았다.** 그러면 위로 올라가도 그대로라
  //    태현님이 지적한 「10층이나 120층이나 똑같다」가 그 사람에게만 그대로 남는다.
  //    0.75 를 쓴 유일한 이유는 「42층 3초컷」 차단이었는데, 그건 이제 **도약이
  //    구조로** 막는다 — 순삭하면 위로 가면 된다(디아블로3 균열·PoE 맵의 방식).
  //    지수로 누르던 일을 구조가 대신 맡으므로 지수를 놓아 준다.
  //  ⚠ 그래도 상수를 둘로 남긴 것은 손잡이를 남기기 위함이다 — 과성장이 다시 문제가
  //    되면 이 한 줄만 0.75 로 올리면 예전 거동으로 돌아간다.
  REF_POW_OVER: 0.38,        // 기대를 앞설 때 — 도약이 구조로 막아 주므로 지수는 낮게
  ...
  atkIndex: function () {
    ...
    //  ⚠⚠ 마력은 **여기서 더하지 않는다** (2026-09-04). 아래에서 `spellSummonMul` 로
    //    한 번만 곱한다 — 양쪽에 다 넣으면 마력에 대한 2차식이 되어 아이템으로 마력을
    //    올린 주술사의 지수가 광전사의 4.8배로 튄다(실측 646 vs 134). 마력은 축이
    //    하나다: 소환수 배수. 평타 몫도 그 배수에서 비율로 떼므로 이미 포함돼 있다.
  ...
  ehpIndex: function () {
    ...
    //  ── 흡혈 (2026-08-03 사용자 신고로 고침) ────────────────────────────────
    //  "파수꾼으로 50층 넘겼는데 흡혈 때문인지 적군이 때려도 95~100% 만 왔다갔다하고
    //   쉽게 끝나버려."
    //
    //  ⚠ 예전 식은 `ib.lifesteal`(**아이템 흡혈만**)을 봤다. 그런데 파수꾼은
    //    `heroes.js` 에서 **타고난 흡혈 0.15** 를 갖는다(광전사 0.10). 흡혈 아이템을
    //    안 사면 `lsMul = 1.0` 이 되어 **흡혈이 난이도에 한 톨도 반영되지 않았다.**
    //    적 공격력(dmgMul)이 내 유효 체력을 따라가야 하는데, 그 유효 체력에서
    //    지속력이 통째로 빠져 있었으니 맞아도 안 죽는 상태가 된다.
    //
    //  ⚠ 계수도 올린다. 흡혈은 **때린 만큼 돌려받는 것**이라 전투가 길어질수록
    //    이득이 커지고, 광역 명중 수만큼 증폭된다(이 저장소 실측 기록: 표기 25% →
    //    실효 79%, 한 방에 평균 3.16기 명중). 1.5 는 그 실측과 안 맞는 값이었다.
    //    다만 무한정 키우면 흡혈 빌드가 못 이기는 벽이 되므로 상한을 둔다.
    var lsRaw = (base.lifesteal || 0) + (ib.lifesteal || 0);
    var lsMul = 1 + Math.min(0.60, lsRaw) * 2.4;
  ...
  //  ── 내 소환수 배수 — **내 능력치를 따라간다** (2026-09-04) ──────────────────
  //  태현님 ①: "주술사 유닛이 너무 약해 나오자마자 1방에 죽어버려."
  //  태현님 ②: "소환수는 층을 따라가기보단 **능력치를 더 따라가게** 해줘."
  //
  //  ①의 원인은 소환수가 성장 추종 밖에 있던 것이었다(적은 13.3배로 자라는데 소환수는
  //  1.0배 → 고성장 90층에서 적 한 대 195 vs 소환 전사 체력 192 = 한 대에 죽는다).
  //  ②는 그 추종의 **기준을 무엇으로 삼을지**를 정한 것이다 — 첫 판은 층 배수
  //  (`modsFor`: 층 선형항 × 성장항 × 층 조건 × 질 배수)를 통째로 썼는데, 그러면
  //  "내가 키운 것"과 "층이 올라간 것"이 섞여 **능력치를 안 올려도 소환수가 세진다.**
  //  지금은 층 항·층 조건·질 배수를 전부 빼고 **내 성장 지수만** 본다.
  //
  //  ⚠ **축은 직결이다** — 내가 단단해지면 소환수도 단단해지고, 내가 세지면 소환수도
  //    세진다. (적 배수는 반대로 엇갈린다: 적 체력이 내 공격을 따라간다. 그건 적을
  //    나에게 맞추는 장치고, 이건 소환수를 **내 분신**으로 두는 장치라 방향이 다르다.)
  //      소환수 체력 ← ehpIndex (내 유효체력 성장: 체력·방어·흡혈)
  //      소환수 공격 ← atkIndex (내 공격 성장: 공격력·쿨감·스킬 진화)
  //  ⚠ **성장이 0이면 정확히 1.000 이다** — 두 지수가 신선한 캐릭터에서 1 이고 층 항이
  //    없으므로. 그래서 회귀 기준선(R-1 등 신선한 캐릭터 측정)은 이 기제가 있든 없든
  //    한 톨도 안 움직인다. 이 성질을 깨지 말 것(수성의 탑 growthIndex 와 같은 규율).
  //  ⚠ 완화(reliefOf)·압박(pressureOf)은 **일부러 뺐다.** 그건 난이도 적응 손잡이지
  //    내 능력치가 아니다 — 막혀서 적이 약해졌다고 내 소환수까지 약해질 이유가 없다.
  ...
  //  왜 넣었나: 이 탑은 위로 **순차로만** 올라갔다(뒤로는 `🔁 지난 층` 이 있었지만
  //  앞으로 건너뛰는 길이 없었다). 그래서 장비가 앞서간 사람은 43·44·45층을 계속
  //  3초컷 하며 걸어 올라가야 했고, 그게 **"42층을 3초만에 깼어"** 신고의 진짜 구조였다.
  //
  //  다른 게임들이 무한 콘텐츠에서 쓰는 답이 이것이다 — 디아블로3 균열·PoE 맵은
  //  곡선을 **고정**해 두고 **몇 단계를 돌지 사람이 고르게** 한다. 거기서 낮은 단계를
  //  순삭하는 것은 버그가 아니라 "위로 가라"는 신호다. 우리에겐 그 '위로 가는 길'이
  //  없어서, 대신 적이 내 능력치를 따라오게 눌러 놨던 것이다(v3.26 재세팅의 배경).
  //
  //  ⚠⚠ **이 기능이 들어가면 능력치 추종이 저절로 안 걸린다.** 앞서간 사람이 자기
  //    수준의 층으로 올라가는 순간 `refAdj` 의 (내 지수/기대 지수)가 1 에 수렴하기
  //    때문이다(실측: 공격지수 30 인 사람이 42층 ×3.25 → 120층 ×1.33 → 160층 1 미만).
  //    그래서 이번엔 REF_POW 계수를 **한 톨도 안 건드렸다** — 구조가 스스로 푼다.
  //
  //  관문을 **보스 층**으로 잡은 이유: 보스가 이미 10층마다이고(`BOSS_EVERY`),
  //  "다음 구간으로 가려면 그 구간의 보스를 이겨라"가 설명이 필요 없는 규칙이다.
  BOSS_GATE_MIN: 10,          // 이 층을 넘긴 뒤에야 도약이 열린다(초보에게 안 보인다)
  nextGateFor: function (floor) {
    var f = Math.max(1, floor || 1);
    var g = (Math.floor((f - 1) / this.BOSS_EVERY) + 1) * this.BOSS_EVERY;
    //  이미 관문에 서 있으면 그 다음 관문이 목표다(같은 층을 도약이라 부르지 않는다).
    if (g <= f) g += this.BOSS_EVERY;
    return g;
  },
  //  도약을 걸 수 있는가 — 최고 기록이 첫 관문을 넘겼고, 목표가 지금보다 위일 때만.
  canPush: function () {
    var rec = this.get();
    return (rec.best || 0) >= this.BOSS_GATE_MIN &&
           this.nextGateFor(rec.floor) > rec.floor;
  },
  //  ── 지금 이 층이 나에게 시시한가 ────────────────────────────────────────────
  //  ⚠ 이 판정이 있어야 도약이 실제로 쓰인다. 디아블로3 방식(곡선 고정 + 사람이 티어
  //    선택)의 약점은 **사람이 올라갈 때를 모르면 낮은 데서 계속 순삭한다**는 것이다.
  //    우리는 이미 「그 층에 도달했을 법한 힘」을 알고 있으므로(expectedIndex), 내가
  //    거기서 얼마나 앞서 있는지로 바로 답할 수 있다 — 새 계측을 만들 필요가 없다.
  PUSH_HINT_AT: 1.6,          // 기대의 1.6배를 넘게 앞서면 권한다
  readyToPush: function () {
    if (!this.canPush()) return false;
    var f = this.get().floor;
    var a = this.atkIndex() / Math.max(0.01, this.expectedIndex(f, 'atk'));
    var e = this.ehpIndex() / Math.max(0.01, this.expectedIndex(f, 'ehp'));
    return Math.max(a, e) >= this.PUSH_HINT_AT;
  },
