...
GAME.CONFIG = (function () {
  ...
  // ── 기준 전장은 **폰 가로**다 (2026-07-31 사용자 지시) ────────────────────────
  //  "PC 에서와 모바일에서 전장 크기가 달라. 모바일 기준으로 똑같게 해줘."
  //
  //  신고가 맞았다. 실측하면 두 프로필은 **모양부터 다른 전장**이었다:
  //    PC  1300×760  가로세로비 1.711 · 사거리 400 이 폭의 30.8% · 높이의 52.6%
  //    폰   808×378  가로세로비 2.138 · 사거리 400 이 폭의 27.5% · 높이의 58.8%
  //  `WORLD_SCALE` 은 **면적**만 맞춰 놨을 뿐 비율은 안 맞췄고, 그 사실이 바로 아래
  //  옛 주석에도 적혀 있었다("가로세로 비까지 같아지는 건 아니다"). 즉 같은 배치도가
  //  기기에 따라 다른 게임이었다 — 회피 게임에서 이건 그냥 다른 난이도다.
  //
  //  이제 폰 가로 전장을 **원본**으로 삼고 PC 는 그것을 그대로 확대한다:
  //  가로세로비를 폰에 맞추고, 거리 배율도 폭 비율만큼만 키운다. 그러면 사거리·이동·
  //  추격이 전장에서 차지하는 비율이 두 프로필에서 **완전히 같아진다**(아래 실측 주석).
  //  ⚠ 세로(PORTRAIT)는 건드리지 않는다 — 화면 비 자체가 세로라 같은 비율을 강요하면
  //    긴 화면에 납작한 띠만 남는다. 그 프로필은 원래부터 따로 재는 대상이다.
  var REF = { w: 808, h: 378 };                       // 기준 전장(폰 가로)
  var REF_SCALE = Math.sqrt((REF.w * REF.h) / (1300 * 760));   // = 0.5560, 기존 폰 값 그대로
  var PC_W = 1300;
  var PC_H = Math.round(PC_W * REF.h / REF.w);        // 1300 × 608 — 폰과 같은 가로세로비

  // 폰 가로는 HUD·조작을 전장 **위에 겹쳐** 올린다(롤토체스 방식) — 높이 390 에
  // 막대를 쌓을 여유가 없다. 그래서 아레나가 캔버스를 거의 다 쓴다.
  var arena = P
    ? { x: 9, y: 9, w: 402, h: 694 }
    : (PHONE ? { x: 6, y: 6, w: REF.w, h: REF.h }
             : { x: 20, y: 20, w: PC_W, h: PC_H });
  ...
  return {
    ...
    //  ── 전투 속도 (2026-08-23 태현님: "데미지 2배·체력 절반, 더 빠른 진행") ────
    //  **유닛에게만** 적용한다(영웅 제외): 유닛 체력 ×HP·유닛이 내는 피해 ×DMG·
    //  유닛이 받는 회복 ×HEAL. 플레이어 체감 = "적이 두 배 빨리 죽고, 적의 공격은
    //  두 배 아프다" — 판은 당겨지고 회피의 값어치는 오히려 올라간다.
    //  2026-08-23 2차 조정(태현님): 적 공격 3 → 1.5, **영웅 공격 ×1.5(HERO_DMG)** 신설.
    //  ⚠ 같은 날 태현님 지시로 **무조작 필패(R-1) 약속은 폐기**됐다:
    //    "무조작 필패 조건은 빼자. 내가 원하는 건 적당한 컨트롤을 해야만 깨는 조건."
    //    regress 의 R-1 은 참고 지표로만 남는다(합격 게이트 아님).
    //  3차 조정(2026-08-23 태현님: "13층 파수꾼이 300씩 4방에 죽는다") — 적 공격
    //  1.5 → 1.2. 성장 추종(dmgMul이 내 유효체력을 따라옴) 위에 PACE 가 얹히는
    //  구조라 탱커 빌드일수록 체감이 배가된다. 여기 숫자가 그 최종 손잡이다.
    PACE: { DMG: 1.2, HP: 0.5, HEAL: 0.5, HERO_DMG: 1.5 },
    ...
    // 기준은 예전 세로 아레나 폭 632(PORTRAIT 전용).
    //
    // 가로 두 프로필(PC·폰 가로)은 **폰을 원본으로 삼아 같은 전장**을 쓴다(위 REF 주석).
    // 가로세로비를 이미 맞췄으므로 배율은 **폭 비율 하나**로 끝난다 — 폭을 맞추면
    // 높이는 저절로 맞는다. 그래서 예전처럼 면적 제곱근을 쓸 필요가 없어졌다.
    //   폰 : REF_SCALE × (808/808) = 0.5560   (기존 값과 동일 — 폰은 무변경)
    //   PC  : REF_SCALE × (1300/808) = 0.8946 (기존 1.0 에서 내려온다)
    // 실측(같은 사거리 400 이 전장에서 차지하는 비율):
    //   변경 전  PC 폭 30.8% · 높이 52.6%   ↔  폰 폭 27.5% · 높이 58.8%
    //   변경 후  PC 폭 27.5% · 높이 58.8%   ↔  폰 폭 27.5% · 높이 58.8%  (완전 일치)
    // ⚠ PC 곡선은 이제 **폰 곡선과 같아진다.** 옛 PC 기준선 표는 이 시점부로 무효다 —
    //   `node tools/regress.js` 로 다시 뽑을 것.
    WORLD_SCALE: P ? arena.w / 632
                   : REF_SCALE * (arena.w / REF.w),
