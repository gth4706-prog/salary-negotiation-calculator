...
GAME.Tower = {
  ...
  // ── 보스 사다리 (2026-08-02 8차 개편) ──────────────────────────────────────
  //  사용자 지시: "얼굴은 보여주지말고 손·발·날개하고 부하들만 미리 보여주자.
  //  깨어나는용하고 너무 다르다. 에그워와 맞게 드래곤 알도 보스에 추가하고,
  //  알이 깨어져서 깨어지는 알(균열 안에서 반짝이는 눈만 보임)도 과정에 추가."
  //
  //  **예고는 크기만 흘리고 정체는 끝까지 숨긴다.** 예전엔 200층 반쪽 얼굴 ·
  //  250층 상반신으로 얼굴을 미리 까 버려서 300층에 남는 놀라움이 없었다.
  //  지금은 이렇게 간다:
  //    50·100·150   발 → 손 → 날개    "얼마나 큰가"만 알려준다(정체는 모른다)
  //    그 사이 10의 배수  권속들        "누구의 부하인가"를 알려준다
  //    200          용의 알            **정체가 알이었다** — 이 게임 세계관의 회수
  //    250          깨어지는 알        균열 틈으로 **눈만** 보인다
  //    300          태초의 용          드디어 얼굴이 나온다
  //  즉 얼굴은 300층까지 아껴 두는 것이 이 사다리의 전부다.
  //  ⚠ 알 두 단계는 세계관 회수이기도 하다 — 이 게임은 "알에서 깨어난 자들의
  //    전쟁"인데(CLAUDE.md) 정작 최종 보스만 알과 무관했다. 계란 부족이 서로
  //    싸우는 동안 산 아래에서 **가장 큰 알**이 깨어나고 있었다는 이야기가 된다.
  BOSS_SCHEDULE: {
    10: 'bossChief', 20: 'bossShell', 30: 'bossNest',
    40: 'bossAshSentry',
    50: 'bossDragonFoot',                                   // ← 첫 부위(크기 예고)
    ...
    200: 'bossDragonEgg',                                   // ← 알(정체 회수)
    250: 'bossDragonCrack',                                 // ← 깨어지는 알(눈만)
