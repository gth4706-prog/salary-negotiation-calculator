...
GAME.TowerRun = {
  ...
  //  ⚠ 2026-08-02 — **25% 너프**(사용자: "골드 획득도 25% 정도 너프해, 너무 잘 나와")
  //    에 이어 **추가 20% 너프**(같은 날 재신고: "20% 를 더 줄여").
  //    24 → 18(-25%) → 14.4 가 아니라 **10.8**(누적 -40%): 18 × 0.75 × 0.8 = 10.8.
  //    위 표(1층 18 · 30층 480 …)는 첫 너프 전 값이고, 지금은 전부 0.6 배다:
  //    1층 11 · 10층 30 · 20층 93 · 30층 288 · 40층 894.
  //    층 돌파 보너스(js/scenes/battle.js 의 floorBase)도 같은 비율로 같이 줄였다 —
  //    한쪽만 줄이면 총량이 이 비율대로 안 줄어든다.
  //  ⚠ 2026-08-02 3차 — **10.8 → 2.4**(사용자: "40탄정도면 500골드정도 먹는게 맞아.
  //    한판에 2천골드나 먹게돼"). 신고가 정확했다. 40층은 **보스 층이라 ×2.5** 가
  //    걸려서, 10.8 기준 한 판에 2,283 골드였다(= 처치 2,019 + 클리어 224 + 돌파 40).
  //    2.4 로 내리면 같은 40층이 **538** 이 된다.
  //  ⚠ 이 값을 다시 만질 때 **보스 층 배수를 잊지 말 것** — 10·20·30… 층은 항상
  //    2.5 배라, "40층 얼마"를 계산할 때 `goldFor(40)` 만 보면 실제의 40% 만 본 것이다.
  //  ⚠ 아이템 사다리(js/towershopitems.js)가 이 곡선에 묶여 있다.
  //    **누적 실측(2026-08-03, 아래 5차 피벗 + 10단 사다리 기준)**:
  //      10층 296 · 20층 938 · 30층 2,324 · 40층 5,315 · 60층 19,121 · 100층 54,693.
  //    풀세트(4슬롯) 가격이 모이는 시점:
  //      1단 62→4층 · 2단 165→8층 · 3단 410→13층 · 4단 785→20층 · 5단 1,550→27층 ·
  //      6단 2,900→34층 · 7단 5,650→41층 · 8단 19,400→61층.
  //    한 단계씩 꾸준히 손에 닿는다(예전 8단 사다리는 3단 14층 → 4단 26층으로
  //    **열두 층이 비어 있었다**). 그래도 벽이 안 되는 이유는 난이도가 성장을
  //    따라오기 때문이다(js/tower.js hpMul/dmgMul — 내가 약하면 적도 약해진다).
  //  ⚠ 4차 — 2.4 → **3.1**(사용자: "500원수준이라했으면 700원수준으로 버프").
  //    40층 한 판 538 → **695**.
  //
  //  ── 5차 · 곡선을 **무릎에서 피벗**한다 (2026-08-03) ─────────────────────────
  //  > "초반을 덜 가난하게하고 인플레이션 안오게 … 30탄까지 너무 가난하고 어려우면
  //  >  초기이탈로 이어지니깐"
  //
  //  실측한 문제: **30층까지 누적 925 골드**였다. 그런데 1~3단계 장비를 다 거치면
  //  637, 가장 싼 스킬 하나가 250 이다 — 30층을 다 올라와야 겨우 장비 3단 + 스킬 하나고
  //  능력치는 한 번도 못 찍는다. 더 나쁜 건 **1~10층 누적 66 골드**로, 1단계 풀세트(62)를
  //  사면 끝이라 초반 열 판 동안 상점에 들어갈 이유 자체가 없었다.
  //
  //  **왜 BASE 만 올리면 안 되는가**: BASE 는 곡선 전체를 평행이동시키므로 후반까지
  //  같은 배수로 부푼다 — 그게 정확히 사용자가 막으라고 한 인플레다. 그래서
  //  **BASE 를 올리는 만큼 RATE 를 낮춰** 두 곡선이 무릎(48층)에서 만나게 했다.
  //    17 × 1.08^47 = 633  ≈  3.1 × 1.12^47 = 638
  //  무릎 뒤는 GOLD_TAIL 이 지배하므로 49층부터 100층까지는 **한 톨도 안 움직인다.**
  //
  //  실측(보스층 ×2.5 포함한 누적):
  //  |            | 10층 | 20층 | 30층 | 48층 한 판 | 100층 한 판 | 100층 누적 |
  //  | 3.1 · 1.12 |   66 |  276 |  925 |       638 |      2,176 |     52,283 |
  //  | 17  · 1.08 |  296 |  938 | 2,324 |      633 |      2,160 |     54,693 |
  //  → 초반 ×4.5 ~ ×2.5, 후반 한 판 ×0.99, 평생 누적 **+4.6%**. 인플레 없이 초반만 올랐다.
  //
  //  ⚠ 이 둘은 **짝으로만** 만진다. BASE 만 올리면 후반이 같이 부풀고, RATE 만 낮추면
  //    후반이 통째로 가난해진다. 바꿀 때는 반드시 `BASE × RATE^47` 이 630 근처인지 확인할 것.
  //  ── 6차 (2026-08-23 태현님: "전반적으로 골드가 너무 부족, 더 후하게") ────────
  //  17 → 22. 이번엔 **일부러 전 구간을 +30%** 올린다(위 5차의 '무릎 고정' 규칙을
  //  의도적으로 벗어난다 — 초반만이 아니라 전체가 가난하다는 지시). 보너스 판
  //  (알지키기·알깨기)의 추가 수입도 이 위에 얹힌다. 무릎 값 22×1.08^47 = 820.
  GOLD_BASE: 22,
  GOLD_RATE: 1.08,
