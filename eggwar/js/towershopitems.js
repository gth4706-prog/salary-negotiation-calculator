  ...
  return {
    ...
    WEAPON_NAMES: {
      vanguard:  ['돌 대검', '청동 대검', '흑요석 대검', '이빨 대검', '뼈 대검',
                          '들소뿔 대검', '질풍 대검', '맹수 대검', '강철 대검', '요석 대검',
                          '바람 대검', '사냥신 대검', '흑철 대검', '엄니 대검', '번개 대검',
                          '용골 대검', '심연 대검', '여명 대검', '용비늘 대검', '태초 대검',
                          '용심장 대검'],
      ranger:    ['돌 활', '청동 활', '흑요석 활', '이빨 활', '뼈 활',
                        '들소뿔 활', '질풍 활', '맹수 활', '강철 활', '요석 활',
                        '바람 활', '사냥신 활', '흑철 활', '엄니 활', '번개 활',
                        '용골 활', '심연 활', '여명 활', '용비늘 활', '태초 활',
                        '용심장 활'],
      warden:    ['돌 방패', '청동 방패', '흑요석 방패', '이빨 방패', '뼈 방패',
                        '들소뿔 방패', '질풍 방패', '맹수 방패', '강철 방패', '요석 방패',
                        '바람 방패', '사냥신 방패', '흑철 방패', '엄니 방패', '번개 방패',
                        '용골 방패', '심연 방패', '여명 방패', '용비늘 방패', '태초 방패',
                        '용심장 방패'],
      shaman:    ['돌 지팡이', '청동 지팡이', '흑요석 지팡이', '이빨 지팡이', '뼈 지팡이',
                        '들소뿔 지팡이', '질풍 지팡이', '맹수 지팡이', '강철 지팡이', '요석 지팡이',
                        '바람 지팡이', '사냥신 지팡이', '흑철 지팡이', '엄니 지팡이', '번개 지팡이',
                        '용골 지팡이', '심연 지팡이', '여명 지팡이', '용비늘 지팡이', '태초 지팡이',
                        '용심장 지팡이'],
      assassin:  ['돌 단검', '청동 단검', '흑요석 단검', '이빨 단검', '뼈 단검',
                          '들소뿔 단검', '질풍 단검', '맹수 단검', '강철 단검', '요석 단검',
                          '바람 단검', '사냥신 단검', '흑철 단검', '엄니 단검', '번개 단검',
                          '용골 단검', '심연 단검', '여명 단검', '용비늘 단검', '태초 단검',
                          '용심장 단검']
    },

    // ── 영웅 컨셉 렌즈 (2026-09-04 태현님 ②③) ────────────────────────────────
    //  "주술사는 아이템도 공격력이 아니라 마력을 올려줘야하고 주술사 컨셉에 맞는
    //   능력치를 올려주도록해 / 암살자도 암살자에 맞는 컨셉아이템으로 꾸려줘"
    //
    //  ## 왜 카탈로그를 복제하지 않았나
    //  이 파일 맨 위가 이미 못박아 뒀다 — 카탈로그를 두 벌로 만들면 **효과값이 조용히
    //  갈라진다.** 그래서 아이템은 한 벌 그대로 두고, **읽는 렌즈**만 영웅마다 바꾼다.
    //  같은 '뼈창'이 광전사 손에선 공격력 +60, 주술사 손에선 마력 +18 이 되는 식이다.
    //
    //  ⚠ 기존 세 영웅(광전사·사냥꾼·파수꾼)은 **렌즈가 없다** = 한 비트도 안 바뀐다.
    //    회귀 기준선(regress·defend-curve·tower-power-curve)이 전부 그 셋으로 잡혀 있다.
    //
    //  ## 주술사 — 공격력이 마력이 된다
    //  `SPELL_PER_DAMAGE` 는 눈대중이 아니라 **성장 배수 파리티**로 잡았다.
    //  보통 영웅은 무기 +w 를 끼면 화력이 `1 + w/D` 배가 된다(D = 기본 공격력, 약 40).
    //  주술사의 화력은 소환수이고 그 배수는 `1 + 마력/100` 이다. 둘이 같아지려면
    //     마력/100 = w/40  →  **마력 = w × 2.5**
    //  그래서 K = 2.5 다. 지수로 자라는 무기(최상급 +13,500)를 끼면 마력도 같은 기울기로
    //  자란다 — 유한한 값으로 잡으면 주술사만 성장 곡선에서 뒤처진다(실측: K=0.3 에서
    //  중반 파리티 0.60~0.67).
    //  ⚠⚠ 그리고 **평타 항을 곱하지 않는다.** 예전 식은 `(D+0.25·마력)/D × (1+마력/100)`
    //    이라 마력에 대해 **2차식**이었다: 최상급에서 atkIndex 가 광전사의 4.8배로 튀어
    //    (실측 646 vs 134) 적 체력이 폭주했다. 마력은 축이 하나다 — 소환수 배수.
    //    평타 몫은 그 배수에서 **비율로** 떼어 온다(`spellBasicAdd`, 0.25 = "아주 조금").
    SPELL_PER_DAMAGE: 2.5,
    SPELL_REF_DAMAGE: 40,
    HERO_ITEM_LENS: {
      //  소환사 — 평타가 가장 약한 영웅이라 공속·치명은 값이 거의 없다. 둘 다 마력과
      //  쿨감(= 소환 주기)으로 돌린다. 체력을 조금 얹는 것은 "서서 시전한다"는 정체성.
      shaman: {
        move: [['damageAdd', 'spellAdd', 2.5],
               ['critAdd', 'spellAdd', 12],
               ['atkspeedAdd', 'cdrPct', 0.6]],
        mul: { hpAdd: 1.10 }
      },
      //  암살자 — 한 방과 기동. 원시 공격력을 조금 내주고 치명·공속·이동으로 받는다.
      //  방어구는 얇게(암살자가 맞고 버티기 시작하면 다른 영웅이 된다).
      assassin: {
        mul: { damageAdd: 0.88, critAdd: 1.60, atkspeedAdd: 1.40,
               speedAdd: 1.25, armorAdd: 0.85 }
      }
    },

    //  아이템 한 개의 **그 영웅 기준** 효과. 렌즈가 없으면 원본 그대로 돌려준다
    //  (객체를 새로 만들지 않는다 — 기존 세 영웅 경로에 할당 하나도 안 는다).
    effOf: function (it, heroKey) {
      if (!it) return null;
      var L = heroKey && this.HERO_ITEM_LENS[heroKey];
      if (!L) return it;
      var o = {}, k;
      for (k in it) o[k] = it[k];
      var i, m;
      if (L.move) {
        for (i = 0; i < L.move.length; i++) {
          var from = L.move[i][0], to = L.move[i][1], rate = L.move[i][2];
          var v = it[from];
          if (!v) continue;
          o[from] = 0;
          if (to === 'cdrPct') {
            //  공속 %p → 쿨감. 곱연산 자리에 합류한다(합계 상한은 itemBonus 가 건다).
            o.cdrMul = (o.cdrMul || 1) * (1 - Math.min(0.45, v * rate / 100));
          } else {
            o[to] = (o[to] || 0) + (to === 'lifestealAdd' ? v * rate : Math.round(v * rate));
          }
        }
      }
      if (L.mul) for (m in L.mul) {
        if (o[m]) o[m] = (m === 'lifestealAdd') ? o[m] * L.mul[m] : Math.round(o[m] * L.mul[m]);
      }
      return o;
    },
    ...
    noteOf: function (it, compact, heroKey) {
      if (!it) return '';
      //  ⚠ 렌즈를 **먼저** 통과시킨다. 안 그러면 주술사 상점이 "공격력 +60" 이라고
      //  적어 놓고 실제로는 마력 +18 을 주는, 값과 설명이 어긋난 상태가 된다.
      it = this.effOf(it, heroKey) || it;
      var ab = function (n) {
      ...
      var n4 = 0;
      ['damageAdd', 'spellAdd', 'hpAdd', 'armorAdd', 'speedAdd', 'lifestealAdd', 'luckAdd',
      ...
      var L = tight
        ? { dmg: '공', sp: '마', hp: '체', arm: '방', spd: '속', ls: '흡', cd: '쿨', lk: '운', as2: '공속', cr: '치명' }
        : (compact
            ? { dmg: '공격', sp: '마력', hp: '체력', arm: '방어', spd: '속도', ls: '흡혈', cd: '쿨', lk: '행운', as2: '공속', cr: '치명' }
            : { dmg: '공격력', sp: '마력', hp: '체력', arm: '방어력', spd: '이동속도', ls: '흡혈', cd: '스킬 쿨', lk: '행운', as2: '공격속도', cr: '치명타' });
      ...
      if (it.spellAdd) p.push(L.sp + sp + '+' + ab(it.spellAdd));
