...
GAME.Tower = {
  ...
  //  ⚠⚠ 2026-09-10 보스 단독 전환 보정 — 최종 **×1.23** (2.4 → 2.95 · 1.65 → 2.02).
  //    처음엔 위협 모델(ehp×dps 합)로 ×1.12 를 얹었는데 **모자랐다.** 같은 rep 으로
  //    A/B 해 보니 10층 돌파가 1.0%(호위 있음) → 5.2%(보스 단독)로 물러져 있었다.
  //  ⚠ 왜 모자랐나: 호위는 위협만 지던 게 아니라 **영웅의 화력과 주의를 나눠 받는**
  //    존재였다. ehp×dps 합은 그 «시간을 먹는» 몫을 못 센다. 위협 모델은 출발점일
  //    뿐이고 최종 값은 **돌파율로** 맞춰야 한다는 뜻이다.
  //  ⚠ 실측(10층 · 숙련0.90 · rep=96): ×1.10 → 2.1%(R-3 -30%p)로 호위 있던 때
  //    (1.0% · -30%p)와 일치. ×1.15 는 0% 로 과보정이다.
  //  ⚠ rep=48 에서는 2% vs 8% 로 보였다가 rep=96 에서 뒤집혔다 — **표본이 작으면
  //    한두 판이 곧 몇 %p 다.** 이 값을 다시 만질 때는 rep 96 이상으로 잴 것.
  BOSS_HP_PREMIUM: 2.95,
  BOSS_DMG_PREMIUM: 2.02,
  ...
  //  ⚠⚠ **2026-09-10 태현님 ①: "보스전에서는 보스 1마리만 나오게 바꾸자".**
  //    호위 예산을 0 으로 만든다. 위 주석의 "난이도는 호위가, 위협은 보스가"라는
  //    분담이 여기서 끝난다 — 이제 보스 혼자 둘 다 진다.
  //  ⚠ 값을 짐작으로 정하지 않았다. `tools/boss-solo-scale.js` 로 «호위가 지던 몫»을
  //    재서 출발점을 잡고, **최종은 돌파율 A/B 로** 맞췄다(아래 BOSS_*_PREMIUM).
  //    총 위협은 그대로 두고 **출처만 호위에서 보스로 옮긴 것**이다.
  //  ⚠ 되돌리려면 이 한 줄만 1.00 으로 돌리고 프리미엄 둘도 같이 되돌릴 것 —
  //    한쪽만 되돌리면 보스 층이 이웃보다 세지거나 물러진다(R-3 가 잡는다).
  BOSS_ESCORT: 0,
