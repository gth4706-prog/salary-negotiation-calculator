...
(function (UI) {
  ...
  UI.FIT = {
    ...
    javelin:      { dx: -0.80, dy: 0.15, w: 0.26, h: 1.95 }   //  2026-08-23 태현님: "창 크기 키워" — 1.5배
  };
  ...
  UI.ABIL_POSE = {
    ...
    //  ── 원이 아닌 광역 셋 (2026-09-08) ───────────────────────────────────────
    //  ⚠ 새 능력 타입을 넣으면 **이 표에도** 넣어야 한다. 빠지면 `apose` 가
    //    undefined 가 되어 모션이 아무 일도 안 하면서 **에러도 안 낸다**
    //    (파수꾼 오라가 그렇게 사라졌던 계열 — art-motion-audit ⑪-2 가 잡는다).
    breath: 'aoeTarget',     // 브레스 — 숨을 모았다 뿜는다(팔·고개를 든 채 유지)
    donut: 'aoeSelf',        // 고리 — 제 몸에서 바깥으로 터진다(눌렸다 펴진다)
    safezone: 'summon'       // 피난처 — 한 자리를 지정해 꽂는다
  };
  ...
  UI.drawUnit = function (g, def, worldX, worldY, color, alpha, facing, walk, idle, opts) {
    ...
    //  황금알(보너스 판) — 전용 벡터. 계약은 보스 분기와 동일({sx,sy,by}).
    if (def.art === 'goldegg' && GAME.GoldEgg) {
      GAME.GoldEgg.draw(g, sx, sy, r, a, idle || (GAME.Iso && GAME.Iso.now) || 0, opts && opts.unit);
