...
//  용 본체(dragon)일 때만 이리로 넘긴다 — 나머지 부위는 전부 벡터로 돌아갔다
//  (8차, js/bossart.js 의 footPart/clawPart/wingPart/eggPart/crackEggPart).
//
...
(function () {
  ...
  //  ⚠ 2026-08-02 6차 — **발/손은 여기서 빠졌다.** 작은 크롭(76×92, 90×88)을
  //    실제 화면에 띄워 보니 배율을 키운 만큼 픽셀이 굵어져 "부위"가 아니라
  //    "깨진 텍스처"로 보였다(스크린샷으로 직접 확인).
  //  ⚠ 2026-08-02 7차 — **부위는 전부 빠졌다.** 사용자 지적: 날개·반쪽 얼굴·
  //    깨어나는 용도 "그냥 짤라서 붙여넣은 것 같다". 맞는 말이었다 — 원본에서
  //    사각형을 오려 쓰는 방식 자체가 어느 배율에서도 '잘린 사진'을 못 벗어난다.
  //    전부 `js/bossart.js` 의 벡터로 옮겼다.
  //  ⚠ 2026-08-02 8차 — 반쪽 얼굴·깨어나는 용은 아예 **없어졌다**(알 두 단계로
  //    교체 — js/tower.js `BOSS_SCHEDULE` 주석 참조). 지금 벡터 부위는
  //    `footPart`/`clawPart`/`wingPart`/`eggPart`/`crackEggPart` 다.
  //    **여기 남는 것은 300층 본체 하나뿐**이고, 그건 원본 그림을 통째로
  //    (1000×605 전체) 쓰기 때문에 잘린 곳이 없다 — 사용자가 "태초의 용 전체
  //    모습은 변함없이 진행"이라고 지시한 그대로다.
  DA.PARTS = {
    // 300층 본체 — 7차에서 배율을 0.72 → 0.92 로 키웠다(사용자 지시: "조금 더
    // 크게"). 위가 화면 밖으로 나가 잘리던 문제는 `draw()` 의 상단 클램프로
    // 따로 막는다(아래 주석 참조).
    dragon: { rect: [0, 0, 1000, 605], scale: 0.92, frame: null, edges: [] }
  };
  ...
  DA.draw = function (scene, kind, sx, sy, r0, alpha, facing, depthHint) {
    ...
    // ── 상단 잘림 방지 (2026-08-02 7차, 사용자 지시: "짤림없게해줘") ──────────
    //  보스는 진형 위쪽에 서므로 `sy`(발밑)가 화면 상단에 가깝다. 배율을 키우면
    //  머리·날개가 캔버스 위로 넘어가 **머리가 잘린 용**이 된다(실측: 300층에서
    //  정확히 그랬다). 배율을 낮추면 "더 크게"라는 요구와 어긋나므로, 대신
    //  **넘치는 만큼 아래로 민다.** 발밑이 그림자에서 조금 떨어지지만, 이 크기의
    //  용은 원래 화면을 뚫고 나온 것처럼 보이는 게 맞아서 어색하지 않다.
    //  ⚠ 렌더 전용 보정이다 — 판정 좌표(`u.x/u.y`)는 한 톨도 안 건드린다.
    var drawY = sy;
    var topPad = targetH * 0.04;
    if (drawY - targetH < topPad) drawY = targetH + topPad;
    img.setPosition(sx, drawY);
