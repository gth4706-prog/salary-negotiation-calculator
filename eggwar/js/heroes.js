...
GAME.HEROES = {
  vanguard: {
    ...
    //  ── 2026-09-11 태현님 «버프 방향성» 재조정 ────────────────────
    //  ① 광전사 — "체력·공격·이속 보통 · **대신 스킬데미지가 강했으면**".
    //  기본값은 이미 «평범하게 균등» 이라 그대로 두고 `skillMul` 만 얻는다.
    //  ⚠ 평타를 깎아서 갑지 않았다 — 스킬 피해가 공격력 계수를 타므로 평타를
    //    깎으면 스킬까지 같이 약해져 요청과 반대로 간다.
  ...
  ranger: {
    ...
    //  ── 2026-09-11 태현님 «버프 방향성» 재조정 ────────────────────
    //  ② 사냥꾼 — "체력 약간 낮음 · 공격력 약간 낮음 · **공격속도 빠름** · 이속 약간 빠름".
    //  쿨 750→600 으로 dps 32.0 → 36.7(+15%), 체력·공격은 조금씩 내려 값을 치렜다.
    //  ⚠ 이속은 158→162 로 **아주 조금만** 올렸다. 2026-07-31 너프(178→158)의 이유가
    //    «사거리 + 최고속도 조합» 이었고, 그건 태현님이 실플레이로 요구한 것이다.
    //    거기로 되돌아가지 않으려면 버프를 **공속 쪽으로** 주는 것이 맞다.
    hp: 840, armor: 20, damage: 23, cooldown: 540, speed: 162, range: 340,
  ...
  warden: {
    ...
    //  ── 2026-09-11 태현님 «버프 방향성» 재조정 ────────────────────
    //  ③ 파수꾼 — "체력 높음 · 방어력 높음 · 이속 낮음 · **대신 가까이 붙었을때
    //  힘이 극대화**". 세 축을 조금씩 더 밀고, 근접 증폭을 새로 준다.
    //  ⚠ 증폭 거리는 **사거리의 배수**다(84 × 2.4 ≈ 202) — px 로 적으면 폰 배율을
    //    안 따라가 조용히 어긋난다(js/combat.js `closeMul` 주석).
    //  ⚠⚠ 2026-09-11 2차 — `closeMul`(근접 스킬 증폭)을 **또다** 태현님이 끩었다:
    //    "스킬피해 추가 빼주고 차라리 **밟는다는 설정**으로 부딪히면 피해입히도록".
    //    그쪽이 맞다 — 증폭은 **안 보이는** 힘이었고(숫자만 커졌다), 밟기는 화면에
    //    그려지는 힘이다. 발밑 고리가 서로에게 «닿으면 아프다» 를 미리 말해 준다.
    //  체력 2000 · 이속 120 — 지시 그대로. 유효 내구 2720 으로 다섯 중 압도적 1위이고,
    //  그게 의도다 — 대신 가장 느리고(120) 평타 dps 는 다섯 중 최하위다.
    //  ⚠ 밟힐 피해 26 · 0.45초마다(= 초당 58) — 평타(26/1초)와 거의 같은 크기다.
    //    붙어 있으면 화력이 두 배가 되는 셈 — "가까이 붙었을때 힘이 극대화"의 보이는 판.
    hp: 2000, armor: 36, damage: 26, cooldown: 1000, speed: 120, range: 84,
    trampleDamage: 26, trampleEvery: 450,
  ...
  shaman: {
    ...
    //  ── 2026-09-11 태현님 «버프 방향성» 재조정 ────────────────────
    //  ⑤ 주술사 — "체력 약간 낮음 · 이속 약간 낮음 · 방어력 낮음 · **평타 매우
    //  약함** · **스킬 쿨타임 빠름**". 적당히 도망가며 소환하는 플레이가 되어야 한다.
    //  평타 16→9(dps 17.8→9.0) · 방어 28→16 · 쿨 0.72배. 평타로는 아무것도 못 한다.
    //  ⚠ 평타를 깎으면 스킬 피해도 같이 내려간다(공격력 계수). 그 몴은 **쿨**로 갚는다 —
    //    소환은 수치가 아니라 «몇 번 깔 수 있느냐» 가 힘이라 쿨이 정확히 그 축이다.
    //  ⚠ 방어 22 → **12** (2026-09-11 태현님: "주술사 기본 방어력을 더 낮춰").
    //    다섯 중 가장 낮다 — "방어력 낮음" 이라는 그의 방향성과 맞는다.
    //  ⚠⚠ 다만 **이걸로 밸런스는 안 바뀐다**(실측). 사냥꾼과의 한 판을
    //    갈라 보면: 소환 켬 주술사 승 잔여 79% · 소환 끕 사냥꾼 승 잔여 61%.
    //    방어력을 12 → 0 으로 내려도 79% → 77% 뿐이다 — **결정하는 것은 소환이다.**
    //    그래서 같은 판에 실시간 소환 화력도 같이 내렸다(js/arenabuild.js RT_SUMMON_MUL).
    hp: 850, armor: 12, damage: 12, cooldown: 950, speed: 146, range: 260,
    skillCdMul: 0.78,
  ...
  assassin: {
    ...
    //  ── 2026-09-11 태현님 «버프 방향성» 재조정 ────────────────────
    //  ④ 암살자 — "체력 낮음 · 이속 높음 · 공격력 높음 · 치명타·치명타닥 높음 ·
    //  **멀리서 원거리스킬 쓰거나 숨어서 한대때리고 도망가야만 쓸것**".
    //  유효 체력 1037 → 858 로 **다섯 중 꼴째**가 된다 — 맞으면 죽는다는 것이
    //  "도망가야만"의 유일한 구현이다(수치로 말하지 않으면 그냥 붙어서 이긴다).
    //  치명타 25→45% · 치명타닥 1.5→1.95 (기본값에 **더해진다**, js/combat.js 주석).
    hp: 760, armor: 18, damage: 32, cooldown: 830, speed: 178, range: 70,
    //  ⚠ 기본값(전 유닛 25% · ×1.5)에 **더해진다** — 태현님 지시는 최종값 기준이다:
    //    "치명타가 200% · 치명타 퍼센트는 30%" → 0.25+0.05 = 0.30 · 1.5+0.5 = 2.0.
    critAdd: 0.05, critMulAdd: 0.50,
...
// ── 스킬별 짧은 이름 (2026-09-09 태현님) ────────────────────────────────────
//  "스킬실행 아이콘은 롤백해서 텍스트로 다시 돌려놔줘. 차라리 스킬별로 3글자 이내
//   확실히 알 수 있는 단어로 쓰자. 글자가 더 어떤 스킬인지 알기 쉬워."
//
//  ⚠ 왜 타입 라벨(`SKILL_TYPE_LABEL`)로는 안 되나: 그건 **타입**의 이름이라
//    광전사의 `대검 돌진` 과 사냥꾼의 `구르기` 가 둘 다 "뛰기" 로 떴다. 영웅이
//    다섯인데 조작부가 거의 같았다. 이 표는 **스킬마다** 다른 말을 준다.
//  ⚠ 3글자 이내 — 조작부 원이 폰에서 지름 26~34px 이라 그 이상은 줄여 맞추다
//    읽을 수 없게 된다(`_addButton` 이 실측으로 폰트를 깎는다).
//  ⚠ 새 글자를 만들지 않는다 — 전부 원래 이름에서 잘라 썼다. 그래야 글꼴
//    서브셋(835자)을 다시 굽지 않아도 되고, 안 그러면 실기기에서 두부가 뜬다.
//    표를 고치면 `node tools/font-audit.js` 를 반드시 다시 돌릴 것.
GAME.SKILL_SHORT = {
  //  광전사
  '박치기': '박치기', '대검 돌진': '대검', '흙먼지 은신': '흙먼지', '폭풍 돌진': '폭풍',
  '부딪쳐깨기': '깨기', '대검 회전': '회전', '후려치기': '후려', '모래 뿌리기': '모래',
  '강타 후려치기': '강타', '광역 휩쓸기': '휩쓸기', '가죽 두르기': '가죽', '광폭화': '광폭화',
  '약초 씹기': '약초', '전장의 함성': '함성', '불굴': '불굴', '바위 내리치기': '바위',
  '가시 함정': '가시덫', '낙석 유도': '낙석', '유성 낙하': '유성', '대지 붕괴': '붕괴',
  //  사냥꾼
  '관통 화살': '관통', '연사': '연사', '불화살': '불화살', '연쇄 사격': '연쇄',
  '독화살': '독화살', '구르기': '구르기', '도약': '도약', '뒷걸음 사격': '뒷걸음',
  '회피 사격': '회피', '장거리 도약': '긴도약', '올가미': '올가미', '끈끈이 덫': '끈끈이',
  '풀숲 위장': '풀숲', '그림자 위장': '그림자', '가시 올가미': '가시줄', '화살비': '화살비',
  '폭풍 화살': '폭풍비', '일격 화살': '일격', '유성비': '유성비', '관통 저격': '저격',
  //  파수꾼
  '갈고리 찍기': '갈고리', '방패 밀치기': '밀치기', '그물 던지기': '그물', '방패 강타': '방패',
  '강철 갈고리': '강철', '방패 세우기': '세우기', '철벽 자세': '철벽', '숨 고르기': '호흡',
  '재생의 숨결': '재생', '불굴의 방패': '불굴', '갈고리 당기기': '당기기', '회전 갈고리': '회전',
  '돌진': '돌진', '대지 돌진': '대지', '강철 사슬': '사슬', '파수 구역': '파수',
  '경계 화톳불': '화톳불', '대지 강타': '강타', '지진 강타': '지진', '불굴의 구역': '구역',
  //  주술사
  '넋 부르기': '넋', '벌떼 부르기': '벌떼', '돌무리 부르기': '돌무리', '덩굴 무리': '덩굴',
  '조상 대군': '대군', '정령의 가호': '정령', '늪의 가호': '늪', '무리 치유': '치유',
  '큰 무리 치유': '큰치유', '조상의 가호': '조상', '늪지기 무리': '늪지기', '족장 무리': '족장',
  '늪지기 떼': '늪떼', '강화된 족장 무리': '강족장', '조상의 부름': '부름',
  '조상의 족장 소환': '큰족장', '조상의 골렘 소환': '골렘', '조상의 파수병 소환': '파수병',
  '조상의 잿날개 소환': '잿날개', '조상의 폭풍용 소환': '폭풍용',
  //  암살자
  '그림자 걸음': '걸음', '되돌기': '되돌기', '표창 돌진': '표창', '덫 놓기 돌진': '덫돌진',
  '그림자 넘기': '넘기', '그림자 영역': '영역', '단검 던지기': '단검', '결계 전개': '결계',
  '그림자 감옥': '감옥', '영역 전개': '전개', '은신': '은신', '연기': '연기',
  '그림자 숨기': '숨기', '풀숲 숨기': '풀숲', '완전 은신': '완전', '처형': '처형',
  '표창 난사': '난사', '분신술': '분신술', '난격': '난격', '마지막 일격': '일격'
};

GAME.skillLabel = function (sk, slot) {
  slot = slot || (sk && sk.slot);
  //  ⚠ **스킬 이름이 먼저다.** 예전에는 슬롯 라벨(R='궁극기')과 타입 라벨이 먼저라
  //    무슨 스킬을 꼈든 같은 글자가 떴다. 표에 없는 스킬만 옛 순서로 되돌아간다.
  var short = sk && sk.name && GAME.SKILL_SHORT[sk.name];
