...
BO.Tutorial = (function () {
  ...
  var steps = [
    { id: 'intro', text: '불이 꺼졌습니다. 당신(청록)은 위쪽 3줄 어딘가, 상대(분홍)는 아래쪽 어딘가에 있습니다. 서로 보이지 않습니다. 한 턴에 행동 2번.', next: true },
    { id: 'move', text: '① 이동 — [🏃 이동] 상태에서 방향키를 누르거나 옆 칸을 짚고 [이동 확정]. 가구는 못 지나갑니다.',
      done: function (ev, byMe, v) { return !!(v && v.myTurn && v.moves > 0); } },
    { id: 'shoot', text: '② 사격 — [🎯 사격]으로 바꾸고 아무 칸이나 짚은 뒤 [발사]. 빗나가도 그 칸에 페인트가 5턴 남습니다 — 그걸 밟는 사람은 발자국을 남기게 됩니다.',
      done: function (ev, byMe) { return !!(ev && byMe && (ev.k === 'miss' || ev.k === 'hit')); } },
    { id: 'lamp', text: '③ 전등 — 바닥의 버튼(🔘)까지 걸어가 밟아 보세요. 불이 켜지고 서로가 보입니다. ⚠ 첫 행동으로 켜야 둘째 행동으로 쏠 수 있습니다.',
      done: function (ev, byMe) { return !!(ev && byMe && ev.k === 'lamp'); } },
    { id: 'lampshot', text: '④ 불빛 아래 — 상대가 보입니다. [🎯 사격]으로 상대 칸을 짚고 [발사]! 행동 하나가 지나면 다시 꺼집니다.',
      done: function (ev, byMe) { return !!(ev && byMe && ev.k === 'hit'); } },
    { id: 'mark', text: '⑤ 발자국 — 맞은 상대는 신발에 페인트가 묻었습니다. 한 칸만 움직이면 발을 뗀 칸에 발자국이 남습니다. 턴을 넘기고 상대를 지켜보세요.',
      done: function (ev, byMe) { return !!(ev && !byMe && ev.k === 'mark'); } },
    { id: 'track', text: '⑥ 추적 — 발자국 바로 옆 칸 어딘가에 있습니다. 후보를 골라 쏘세요. 맞히면 끝!',
      done: function (ev, byMe) { return !!(ev && byMe && ev.k === 'hit'); } },
    { id: 'end', text: '끝. 핵심은 하나 — 모르겠으면 버튼을 하러 가고, 움직이면 페인트를 밟고, 밟으면 발자국이 남습니다. 이제 실전으로.', next: true, final: true }
  ];
