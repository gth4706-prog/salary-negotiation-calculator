...
GAME.RtPrepScene.prototype._refresh = function () {
  ...
  //  ── 자동으로 넘어가기 전에 **먼저 말한다** (2026-09-13 태현님 ②) ────────────
  //  ⚠ 시간이 다 되면 이 흐름은 사람 대신 고르고(영웅 단계) 사람 대신 확정한다
  //    (상점 단계). 그 자체는 필요한 장치다 — 안 그러면 자리를 뜬 사람 하나가
  //    상대의 판을 통째로 붙든다. 문제는 **예고 없이** 그런다는 것이었다:
  //    태현님이 «준비 안 됐는데 강제시작» 으로 신고한 그 화면이다.
  //  ⚠ 숫자만 빨개지는 것으로는 부족하다 — 무엇이 일어날지를 글자로 말해야
  //    사람이 «지금 눌러야 한다» 를 안다.
  //  ⚠ **한 줄로 붙인다.** 줄을 늘리면 폰(H 390)에서 아래 버튼과 겹친다 —
  //    이 저장소가 반복해서 겪은 자리다(같은 화면의 스킬 줄이 이미 그랬다).
  var _warn = (s > 5) ? ''
