...
BO.Tutorial = (function () {
  ...
  var steps = [
    { id: 'intro', text: '불이 꺼졌습니다. 8×8 방. 당신(청록)은 위쪽 2줄, 상대(분홍)는 아래쪽 2줄 어딘가. 보이는 건 당신이 바라보는 쪽 부채꼴뿐 — 가구도 상대도 그 안에 들어와야 보입니다. 한 턴에 행동 2번.', next: true },
    { id: 'move', text: '① 이동 — [🏃 이동] 상태에서 방향키를 누르거나 옆 칸을 짚고 [이동 확정]. 움직인 방향으로 시야가 열립니다. 모르는 칸으로 가다 가구에 부딪히면 행동 하나를 잃고 그쪽을 보게 됩니다.',
      done: function (ev, byMe, v) { return !!(v && v.myTurn && v.moves > 0); } },
    { id: 'shoot', text: '② 사격 — [🎯 사격]으로 바꾸고 아무 칸이나 짚은 뒤 [발사]. 페인트는 야광이라 판이 끝날 때까지 남고, 맞은 자리가 무엇이었는지 얼룩 모양대로 드러납니다. 얼룩을 밟은 사람은 야광이 묻어 윤곽이 보입니다.',
    ...
    { id: 'mark', text: '⑤ 윤곽과 발자국 — 맞은 상대는 야광이 묻어 윤곽이 보입니다. 움직이면 꺼지고, 한 칸만 움직이면 발을 뗀 칸에 발자국이 남습니다. 턴을 넘기고 상대를 지켜보세요.',
      done: function (ev, byMe) { return !!(ev && !byMe && ev.k === 'mark'); } },
    { id: 'track', text: '⑥ 추적 — 발자국 바로 옆 칸 어딘가에 있습니다. 후보를 골라 쏘세요. 맞히면 윤곽이 보이니 한 발 더!',
      done: function (ev, byMe) { return !!(ev && byMe && ev.k === 'hit'); } },
    { id: 'end', text: '끝. 핵심 넷 — 바라보는 쪽만 보인다 · 야광 얼룩은 지워지지 않는다 · 맞거나 밟으면 윤곽이 보이니 움직여라 · 모르겠으면 버튼. 이제 실전으로.', next: true, final: true }
  ];
