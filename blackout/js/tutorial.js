...
BO.Tutorial = (function () {
  ...
  var steps = [
    ...
    { id: 'box', text: '③ 보급 상자 📦 — 세 턴마다 바닥에 상자가 떨어집니다. 자리도 내용물도 둘 다 보이는 유일한 물건이에요. 밟으면 줍고(손이 비어 있을 때만), [📦 도구]로 던지면 겨눈 칸 둘레가 통째로 칠해집니다. ⚠ 줍는 순간 상자가 사라지니 그 자리가 상대에게 드러납니다 — 도구를 얻는 대신 위치를 파는 거래입니다. 지금 주우러 가도 되고, [다음]으로 넘어가도 됩니다.', next: true,
      done: function (ev, byMe) { return !!(ev && byMe && ev.k === 'take'); } },
    ...
    { id: 'end', text: '끝. 핵심 다섯 — 바라보는 쪽 두 칸만 보인다 · 야광 얼룩은 지워지지 않는다 · 젖은 신발은 걸음마다 방향이 있는 발자국을 남긴다(맞았으면 두 칸 도망) · 📦 상자는 둘 다 아는 유일한 목적지다(가면 만난다) · 모르겠으면 버튼. 이제 실전으로.', next: true, final: true }
  ];
