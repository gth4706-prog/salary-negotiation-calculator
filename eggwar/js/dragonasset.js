...
(function () {
  ...
  //    "깨진 텍스처"로 보였다(스크린샷으로 직접 확인, scale 1.75/1.65 시도에서
  //    렌더 자체가 실패하기도 했다 — 아래 셋째 사고 참고). 사용자 지시("게임과
  //    어울리게")에 따라 이 둘은 `js/bossart.js` 의 `footPart`/`clawPart` 로
  //    되돌렸다(재 파수병·잿날개와 같은 벡터 재료). 300층 본체와 150/200/250층은
  //    크롭이 커서 배율이 작아도 되므로(그래서 화면에서 멀쩡했다) 래스터로 남는다.
  DA.PARTS = {
