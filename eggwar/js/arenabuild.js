...
GAME.ArenaBuild = {
  ...
  //  ⚠⚠ 그보다 먼저 — **관문이 주술사를 평타만으로 재고 있었다.** rt-balance-audit 의
  //    autoSkills 가 `damage>0||dps>0` 로 소환 스킬을 통째 걸러냈다(2026-09-10 에
  //    rt-hero-matrix 에서 고친 것과 **같은 버그가 두 번째 도구에** 있었다).
  //    고치니 교차 대진 80판 주술사 **0승 → 32승**. 즉 2026-09-03 에 적은 `damage: 3.0`
  //    은 제품이 약해서가 아니라 **도구가 그의 화력을 안 세서** 붙은 값이었다.
  //    그 값을 이제 내린다(3.4 → 2.0) — 태현님 지시("평타는 정말 약하게")와도 맞는다.
  //  · 실시간 체력 ×1.40 — 영웅 버프로 TTK 가 15초로 짧아졌다(목표 18~45).
  //  · 파수꾼만 **0.66** — 기본 체력이 1000→2000 으로 두 배가 됐기 때문이다.
  //    탑은 지시대로 2000 으로 두고(그 체감을 원하셨다), 실시간만 여기서 줄인다.
  //  ⚠ 남은 관문 2건은 전부 **방어 몰빙 주술사**다(거울전에서 armorMax 가 balanced 를
  //    잔여 98% 로 이긴다). 원인이 구조적이다 — 평타가 약해졌으니 공격 아이템이
  ...
  //  ── 기습 일격의 실시간 감쇠 (2026-09-04 암살자 닌자 개편) ────────────────────
  //  은신 해제 타격 ×2.0~3.2 는 **탑 기준**으로 잡은 값이다. 1:1 실시간에서는
  //  그 한 방이 판을 그대로 끝내 `rt-balance ②`(승자 잔여 ≤55%)가 깨졌다
  //  (실측: 승자 잔여 62% · 초과 4판). 파수꾼 R 오라와 같은 처리 —
  //  **탑은 그대로 두고 실시간만** 여기서 깎는다.
  //  ⚠ 배수 전체가 아니라 **초과분**만 줄인다(2.0 → 1+1.0×k). 배수 자체에 곱하면
  //    k=0.5 일 때 1.0 아래로 내려가 기습이 오히려 약해지는 구간이 생긴다.
  RT_AMBUSH_K: 0.5,

  //  ── 소환의 실시간 감쇠 (2026-09-10 태현님 "주술사가 너무 쓸") ───────────
  //  소환수 수치는 **탑 기준**이다. 탑은 적이 여러기라 소환수가 분산되고 성장
  //  추종(`summonModsFor`)이 난이도와 같이 오른다. 실시간 1:1 은 둘 다 없다 —
  //  상대가 하나라 무리가 **한 명에게 전부** 붙고, 성장이 없으니 상대만 그대로다.
  //  실측(`rt-hero-matrix`, 시드 6 × 자리 2): 주술사 승률 **100%** · 잔여 체력 71% ·
  //  판이 4.7~7.4초에 끝난다(다른 영웅끼리는 14~27초). 태현님 체감 그대로다.
  //  ⚠ 파수꾼 R 오라·기습 일격과 **같은 처리** — 탑은 한 줄도 안 건드리고 실시간만 깎는다.
  //  ⚠ 소환 **수**를 줄이지 않는다. 수를 줄이면 "무리를 부른다"는 주술사의 정체성이
  //    사라진다. 한 기씩의 **체급**만 낮춘다 — 여전히 무리가 나오고, 다만 녹는다.
  //  채택값 — 승률이 아니라 **판 길이와 잔여 체력**로 고른다. 하네스의 자동조종은
  //  무리를 헤치는 법을 모르고 그대로 걸어들어가서 주술사의 **이기느냐**는 과대평가된다.
  //  대신 «몇 초에 끝나느냐 · 이긴 쪽이 얼마나 멀줥하냐» 는 조종 편향이 작다.
  //    전:  4.7초 · 잔여 70% · 다른 넷이 전부 0:12   ← 태현님 체감과 같다
  //    후: 13.2초 · 잔여 37% · 파수꾼과 6:6   (다른 영웅 승자 잔여 16~26% · 14~27초)
  //  ⚠ 단일 축으로는 안 듣는다 — 체력만 0.25 까지 깎아도 승률 98%, 화력만 0.40 에서도 100%.
  //    쓰러지는 것과 녹이는 것을 둘 다 달래야 무리가 «시간을 버는 벽» 이 아니게 된다.
  //  ⚠⚠ 2026-09-11 3차 — 화력 0.95 → **0.55**. 태현님이 짚은 것은 방어력이었지만,
  //    갈라 재보니 그 대진을 결정하는 것은 **소환**이었다:
  //      소환 켬 → 주술사 승(잔여 79%) · 소환 끕 → 사냥꾼 승(잔여 61%)
  //      방어 12 → 0 으로 내려도 79% → 77% (사실상 무관)
  ...
  //  `rt-balance ②`(승자 잔여 ≤55%)가 깨졌다. **대조군으로 원인을 특정했다** —
  //  W 만 옛 mark 로 되돌리면 게이트가 통과한다(11/11). 즉 다른 변경 탓이 아니다.
  //
  //  왜 깨졌나: 옛 표식은 **대상을 따라다녔고** 영역은 **자리에 고정**된다. 1:1 은
  //  서로 크게 움직여서 탑 기준 반지름(110~165)이 결투를 거의 못 담고, 그래서 증폭이
  //  붙었다 말았다 하며 판이 한쪽으로 크게 기울었다(암살자가 이길 때 57% · 질 때 56%).
  //
  //  ⚠ **스칼라로는 못 잡는다** — 실제로 스윕해 봤다: damage 를 올리면 이기는 쪽이,
  //    내리면 지는 쪽이 넘치고(1.3/1.32/1.35/1.4/1.45 전부 실패), hp 도 같았다.
  //    기습 배수를 0 까지 꺼도 그대로였다(= 기습이 원인이 아니다).
  //    분산의 원천은 **증폭이 붙느냐 마느냐**였지 증폭의 크기가 아니었다.
  //  → 배수를 깎는 대신 실시간에서만 **반지름을 키워 닿게** 만든다. 그래야 스킬의
  //    성격(영역을 잡고 그 안에서 싸운다)이 안 바뀐 채로 안정된다. 실측 1.8 에서 11/11.
  RT_ZONE_K: 1.0,     // 배수는 탑 값 그대로(1.0 = 감쇠 없음). 손잡이만 남겨 둔다.
  RT_ZONE_R: 1.8,     // 반지름만 키운다 — 이 값이 게이트를 통과시킨 손잡이다.

  //  ── 영역 안 회피율의 실시간 감쇠 (2026-09-04) ──────────────────────────────
  //  회피는 1:1 에서 가장 강한 축이다(맞고 안 맞고가 곧 판이다). 게다가 실시간은
  //  위 RT_ZONE_R 로 영역이 1.8배라 **결투 내내** 걸린다 — 탑 값(12~25%)을 그대로
  //  쓰자 rt-balance ② 가 62% 로 깨졌고, 0.4 로 깎아도 62% 였다(스윕으로 확인).
  //  → **실시간에서는 아예 끈다(0).** 탑에서는 태현님 요청대로 그대로 돈다.
  //  ⚠ 0 이 아닌 값을 넣고 싶으면 rt-balance ② 를 반드시 다시 돌릴 것.
  RT_ZONE_EVADE_K: 0,
