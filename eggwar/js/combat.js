...
GAME.Combat = {
  ...
  //  ── 방어 태세 반사의 세기와 뚜껑 (2026-09-05 태현님 ⑤) ──────────────────────
  //  `REFLECT_MUL`      유닛 def 의 `guard.reflect` 에 곱하는 전역 배율.
  //                     보스마다 잡아 둔 0.50~0.65 의 **상대 관계는 그대로 두고**
  //                     전체를 한 번에 낮춘다(개별 값을 손대면 보스 간 성격이 흐려진다).
  //  `REFLECT_MAX_FRAC` 반사 한 번이 가져갈 수 있는 **최대체력 비율**의 상한.
  //                     0.28 이면 온전히 맞아도 네 번은 버틴다 — 한 방에 죽지는 않되
  //                     "때리면 안 되는 시간"이라는 교훈은 그대로 아프다.
  //  ⚠ 뚜껑이 **본체**다. 배율만 낮추면 공격력을 더 올린 순간 같은 즉사로 돌아온다
  //    (반사는 내 공격력에 비례하고 내 체력은 다른 축이다 — 실측으로 확인).
  REFLECT_MUL: 0.55,
  REFLECT_MAX_FRAC: 0.28,
  ...
  applyDamage: function (unit, dmg, source, state, opts) {
    ...
    if (unit._guardOn && unit.def && unit.def.guard && !(opts && opts.noReflect)) {
      ...
      if (source && source.alive && source !== unit && source.side !== unit.side &&
        ...
        //  ── 반사 상한 (2026-09-05 태현님 ⑤) ─────────────────────────────────
        //  "그 공격데미지 그대로 돌려받는건 너무 치명적이야 … 130층에 내 평타 한번
        //   반사당했다고 내가 1방에죽는건 말이안돼"
        //  ⚠⚠ 이건 값이 큰 문제가 아니라 **구조** 문제였다. 반사량은 내 공격력에
        //    비례하는데 내 체력은 **다른 축**이라, 지수 경제에서 공격력이 체력보다
        //    빨리 자라는 순간(= 화력 몰빵) 반사는 반드시 즉사가 된다.
        //    실측(130층 보스, 반사 0.50): 균형 9k/14k → 24% · 화력 40k/12k → **100% 즉사**
        //    · 80k/12k → 즉사 · 150k/15k → 즉사. 비율만 낮춰도 공격력을 더 올리면
        //    같은 자리로 돌아온다 — 그래서 **최대체력 비율로 뚜껑**을 씌운다.
        //  ⚠ 뚜껑은 '실제로 깎일 체력' 기준이다. 방어 무시분(아래 되곱하기)을 먹이기
        //    **전에** 걸어야 방어력이 높은 사람만 뚜껑이 헐거워지는 일이 없다.
        //  ⚠ 기제 자체는 살린다 — 한 번에 죽지는 않되 세 번 맞으면 죽는다.
        //    "멈출 줄 아는 것이 실력"이라는 태세의 뜻은 그대로다.
        var net = attempted * gd.reflect * this.REFLECT_MUL;
        var rcap = (source.maxHp || 0) * this.REFLECT_MAX_FRAC;
        if (rcap > 0 && net > rcap) net = rcap;
        var back = Math.max(1, Math.round(net * (100 + srcArmor) / 100));
  ...
  update: function (state, dtMs) {
    ...
    //  ── 소환수 시체 치우기 (2026-09-13 태현님 ③ 주술사 둘이면 렉) ──────────────
    //  ⚠⚠ 죽은 유닛은 `state.units` 에 **영원히 남는다.** 진형 20기짜리 판에서는
    //    안 보이지만 소환은 판 내내 계속 나오므로 배열이 단조 증가한다 — 주술사
    //    둘이 붙은 90초 판 실측 **112기(살아있는 것은 11기, 시체 101기)**.
    //    이 배열을 매 틱 훑는 루프가 스무 곳이 넘고 그중 둘은 이중 루프(O(n^2))라,
    //    «스킬 남발 때도 난전 때도 아닌데 느려진다» 가 된다.
    //  ⚠ **소환수만** 치운다. 진형·영웅·보스 시체는 그대로 둔다 — 시간 초과 판정
    //    (`ratio`)이 진영 최대 체력 합을 세고, 보스는 사망 모션 때문에 씬이 붙든다.
    //    소환수는 둘 다 해당이 없다(영웅이 있는 판은 `ratio` 가 영웅 체력만 본다).
    //  ⚠ 죽은 **그 틱에는 안 치운다** — 사망 효과(결속·폭심·노른자)가 그 틱에 돈다.
    //    한 틱 뒤인 여기서 치우면 그 뒤를 보는 코드가 없다.
    //  ⚠ 록스텝에서도 안전하다 — 양쪽이 같은 코드로 같은 틱에 같은 것을 치우고,
    //    배열 순서가 보존되므로 난수를 쓰는 루프의 소비 순서도 같다.
    for (var _cu = state.units.length - 1; _cu >= 0; _cu--) {
      var _cd = state.units[_cu];
      if (_cd.summoned && !_cd.alive) state.units.splice(_cu, 1);
    }
