...
//  ⚠ 2026-08-07 — 옛 주석은 "지면 레벨도 같이 사라진다"를 근거로 삼았는데, 그 규칙이
//    사라졌다(`DefendTower.fail()` 은 이제 아무것도 안 지운다). 저장 위치를 여기 두는
//    이유는 이제 **수성의 탑 안에서만 듣는 값**이기 때문이다 — 대전·통곡의 탑은
//    각자의 레코드를 쓴다. 영구 성장이 1회차를 무의미하게 만드는 문제는
//    `DefendTower.heroModsFor` 의 성장 추종이 따로 푼다.
GAME.UnitLevel = {
  MAX: 5,
